...
<CommentaireGit>
[BFS-69BD] AudioChannel1Routine_69BD - Dispatch $06 si pas CENTER
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index 3b0522e..e935b06 100644
...
index ea598ae..6a0083f 100644
--- a/src/bank_003.asm
+++ b/src/bank_003.asm
@@ -9629,25 +9629,16 @@ AudioChannel1Routine_699E:
...
+    db $23, $23, $13, $00               ; $69B9-$69BC: Envelope sequence (index 10-13, $00 = end)
 ...
 ; --------------------------
@@ -9661,14 +9652,14 @@ AudioChannel1Routine_69BD:
     ret z
...
@@ -9682,7 +9673,7 @@ AudioChannel1Routine_69CB:
     ld c, [hl]
     inc [hl]
     ld b, $00
-    ld hl, $69af
+    ld hl, AudioData_003_69AA + 5  ; Point to envelope table at $69AF
     ...
     ld a, [hl]
     and a
diff --git a/src/bank_003.asm.current b/src/bank_003.asm.current
new file mode 100644
index 0000000..913380e
--- /dev/null
+++ b/src/bank_003.asm.current
@@ -0,0 +1,14636 @@
...
+    rst $38
+    rst $38
diff --git a/src/bank_003.asm.with_my_changes b/src/bank_003.asm.with_my_changes
new file mode 100644
index 0000000..913380e
--- /dev/null
+++ b/src/bank_003.asm.with_my_changes
@@ -0,0 +1,14636 @@
+SECTION "ROM Bank $003", ROMX[$4000], BANK[$3]
+
+; Constante pointant vers data table (offset du jr à $4E73)
...
<ReflexionAgent>
 ...
 ^[[C^[[C^[[C^[[C│ Je vais analyser le nœud **$69E9** - DispatchAudioWave_Setup qui dispatche vers $69F1.
│ **Point clé** : Comprendre ce mécanisme de dispatch audio et documenter la configuration des paramèt
│ Je vois que $69F1 semble être dans une zone mal désassemblée. Laisse-moi chercher ce qui existe déjà
│ Parfait ! Je vois maintenant que $69E9 est effectivement du code propre (ld a, $06 + ld hl, $69f1), 
│ - Jump vers `DispatchAudioCommand`
│ Vérifions maintenant les références.
│ Parfait ! $69E9 est référencé depuis `AudioChannel1StatusTable` comme entrée d'index 7 (après $69BD,
│ Maintenant vérifions que le build passe toujours :
│ Ah oui, il faut déclarer ce label ! Il doit correspondre à la zone mal désassemblée qui suit. Laisse
│ Les données commencent à $69F1 et vont jusqu'à... trouvons la fin en regardant le code qui suit :
│ Ligne 9731 (`set 0, a`) semble être du vrai code ($C7 = set 0, a). Vérifions :
│ Bon, cherchons mieux. Analysons les patterns autour comme ceux déjà documentés :
│ Je vois ! Les patterns précédents font 5 octets (format NR10-NR14) ou 9 octets. Analysons la zone ma
│ Attends, je vois un pattern : à $69EB commence déjà des données ! Reculons un peu :
│ Ah ! À $69E6 il y a `02 32 49` qui semble être le début. Refaisons :
│ Ah je vois le problème ! Le xxd montre `FE 02 31 49...` mais le code désassemblé dit `ld a, $06 / ld
│ - `21 F1 69` = ld hl, $69F1
│ - `C3 B9 6A` = jp $6AB9 (DispatchAudioCommand)
│ - Ensuite `00 30 F0 A7...` = données
│ Excellent ! Donc le jp $6AB9 = jp DispatchAudioCommand. Maintenant, après le jp (à $69EF = $E9E9 + 6
│ Je vois ! À $EA0F (`CD 0C 6B`) = `call $6B0C` → c'est du code ! Donc les données vont de $E9EF à $EA
│ Hmm, attendez... $B9 $6A, c'est pas au début ! Laisse-moi recompter :
│ Parfait ! Les données commencent bien à $69F1, exactement l'adresse chargée dans HL. Maintenant liso
│ Parfait ! Maintenant je dois reconstruire cette zone. Les lignes 9707-9729 dans le ASM sont mal désa
│ Attendez, la ligne 9731 `set 0, a` = $C7 = fait partie du pattern de données ! Vérifions :
│ Bon ! Donc le vrai code `call UpdateAudioFrameCounter` commence à $6A0F. Cela signifie que les donné
│ Parfait ! Maintenant vérifions que tout compile correctement :
│ 2. **AudioWaveformPattern_69F1** ($69F1) - référencé par le `ld hl`
│ Cherchons d'autres références potentielles :
│   "summary": "Configure et dispatche une commande audio waveform ($06) avec pattern de 30 octets à $
│ }
│ ```
