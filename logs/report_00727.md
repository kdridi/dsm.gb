...
<CommentaireGit>
[BFS-757C] Tile data référencée par Level3MapData
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index c4207a8..320c3c8 100644
...
index 4951add..a2e1fb1 100644
--- a/src/bank_001.asm
+++ b/src/bank_001.asm
@@ -1862,28 +1862,110 @@ DataPadding_4a4f:
     nop
     nop
...
+    or e
 ...
 DataZone_4acb:
     ld c, $e7
@@ -3296,7 +3378,7 @@ SharedTilesetData_578:
...
+    dw $56CD, TileGraphic_5ABB, $6048, $56CD, $574A, $57EB, $5D32, $586F
     ...
     db $FF  ; Terminateur
@@ -3305,17 +3387,14 @@ SharedTilesetData_024:
...
@@ -3366,6 +3445,8 @@ Level3MapData:  ; $5665
...
+    ; Note: $757C = Level3TileData27 (1750 bytes de tile data RLE, actuellement mal désassemblé)
+    ;       TODO BFS: Reconstruire comme "db" statements (voir $7442 Level3TileData20 pour exemple)
     db $FF  ; Terminateur
 
 ; ==============================================================================
@@ -4686,15 +4767,6 @@ MapTileData_5CA6:  ; ($5CA6)
     ld e, l
     cp $f1
     ld e, l
...
-MapTileData_5D8A:  ; $5D8A - pointe vers le byte $F1
     ...
     ld e, l
     cp $f1
@@ -4965,34 +5037,16 @@ MapTileData_5D8A:  ; $5D8A - pointe vers le byte $F1
     ld h, a
     ld l, c
     ld b, c
...
+    ld b, c
     ...
     add c
     add d
@@ -6156,39 +6210,58 @@ EntityAnimationData_5EC4:  ; $5EC4
     ldh [c], a
     ld l, l
     ld l, [hl]
-
...
+    cp $03
     ...
     ld c, d
     ld e, [hl]
@@ -6252,40 +6325,56 @@ EntityData_640D:  ; $640D (pointe vers le $03 du 'cp' ci-dessus)
     ld h, c
     db $f4
     call nz, $504c
...
+    ld l, [hl]
     ...
     ld e, l
     ld c, d
@@ -6519,18 +6608,11 @@ AnimationFrameData_6463:
     ld a, a
     ld a, a
     ld l, [hl]
...
+    cp $02
     ...
     add d
     ldh [c], a
@@ -6686,711 +6768,1999 @@ TileGraphic_65D3:  ; $65D3
     ld [hl], e
     ld h, e
     ld h, h
...
+    cp $08
     ...
     ld h, a
     ld h, a
@@ -7581,29 +8951,6 @@ Level3TileData8:  ; $7038 - Point d'entrée alternatif dans Level3TileData5
     ldh [c], a
     db $fd
     ld h, a
-
...
-;  à cause du chevauchement du désassemblage - voir commentaire ci-dessus]
     ...
     ld sp, $697f
     ld a, a
@@ -8083,27 +9430,6 @@ ProcessValidation_7371:
     pop af
     ld e, l
     cp $05
-
...
-Level3TileData17:  ; $737A (devrait être $7379) - Tile data (mal désassemblée)
     ...
     ld a, a
     ld l, c
diff --git a/src/bank_001.asm.pre-bfs-757C b/src/bank_001.asm.pre-bfs-757C
new file mode 100644
index 0000000..5b96bb0
--- /dev/null
+++ b/src/bank_001.asm.pre-bfs-757C
@@ -0,0 +1,12304 @@
...
│ Bon, je vois que cette zone est mal désassemblée (commentaire ligne 3528). Cherchons la zone autour 
│ Maintenant regardons l'endroit exact dans le fichier source pour voir le code désassemblé:
│ Utilisons une approche plus simple - calculons la position dans le fichier:
│ Le fichier a seulement 11750 lignes. Cette zone est encore désassemblée comme du code. Cherchons dan
│ $791A - $56CB (TilesetData_Padding) = $224F = 8783 octets depuis le début de cette zone
│ Analysons les vraies données avec xxd:
│ Longueur: $79E9 - $791A = $CF = 207 octets
│ Analysons le pattern des données. Je vois une structure répétitive qui ressemble au format d'entités
│ Bon! Je vois que $791A est référencé 2 fois dans Level3EntitiesData (ligne 3464). Maintenant analyso
│ Parfait ! Je vois le format. Maintenant, créons la reconstruction de Level3EntityData09. Utilisons l
│ Excellent! Maintenant je dois trouver où insérer ce bloc dans le fichier source. Cherchons la fin de
│ - EntityData08: $79E9-??? 
│ Attends, l'ordre dans le fichier binaire est 07, puis 09, puis 08. Vérifions:
│ Parfait! Donc EntityData07 fait 349 bytes (pas 556 comme indiqué). Je dois corriger ça aussi. Cherch
│ Parfait! Je vois qu'à partir de la ligne 10263, on a exactement les mêmes données que Level3EntityDa
│ Regardons où se termine EntityData07 réellement:
│ 4. Le reste devient EntityData08
│ Vérifions que la ligne 10262 contient bien la fin:
│ Parfait! À la ligne 10262, j'ai `$6F, $82, $82, $6F, $3B, $FE, $03, $3B`. Le premier `$FE` termine E
│ Commençons par éditer le fichier. D'abord, je dois couper la ligne 10262 au bon endroit:
│ Bon! Maintenant je dois couper EntityData07 après le $FE à la ligne 10262, et insérer EntityData09:
│ Excellent! Maintenant je dois continuer à remplacer les lignes suivantes qui font partie de EntityDa
│ Parfait! Je vois que EntityData09 se termine à la ligne 10297 avec `$6F, $E2, $6F, $3B`. Ensuite lig
│ Les données de EntityData09 vont des lignes 10271-10297. Je dois remplacer toutes ces lignes par les
│ Excellent! Maintenant je dois reconstruire EntityData08. Extrayons les données depuis le binaire:
│ Parfait! Maintenant je dois trouver où se termine la zone mal désassemblée de EntityData08 pour la r
│ EntityData08 a 201 bytes, donc 201 opcodes mal désassemblés. Les données se terminent par `$FE`. Che
│ Je vais remplacer toute la section mal désassemblée d'un coup. D'abord, trouvons où EntityData08 se 
│ Donc EntityData08 se termine par `$FE`. Ensuite commence EntityData10 à $7AB2 avec `$02, $3B, $5B`. 
│ Je vais utiliser une approche différente. Comptons le nombre de lignes depuis le début de EntityData
│ Parfait! EntityData08 se termine par `$FE`. Maintenant je vais chercher dans le fichier source le de
│ Le problème est que je ne peux pas facilement identifier où se termine EntityData08 dans le code dés
│ Continuons à lire pour trouver où se termine EntityData08:
│ Je vais reconstruire complètement le fichier en utilisant une approche plus directe. Trouvons où com
│ Cherchons les données `02 3b 5b a2 5e` dans le fichier source:
│ Je vais utiliser une stratégie différente. Je vais chercher `cp $05` et `cp $04` qui apparaissent da
│ Donc EntityData08 se termine par `$3B, $FE` à $7AB1. EntityData10 commence par `$02, $3B, $5B, $A2` 
│ Non, ce n'est pas le bon. Je vais simplement chercher la dernière occurrence de `cp $02` avant un `c
│ Mais nous cherchons juste `FE` (sans l'argument).
│ En fait, dans les données binaires, le dernier $FE est à $7AB1. Cela correspond à un opcode `cp` ave
│ = `FE 02 3B 5B 33 5E 6F 6F E2 6F 3B`
│ Attendez, `inc sp` = `$33`, `ld e,[hl]` = `$5E`. Vérifions avec les données xxd:
│ Donc EntityData08 va de la ligne 10305 à 10562 (le `dec sp` après `cp $02`). La ligne 10563 (`cp $05
│ Maintenant je peux faire le remplacement:
│ Parfait! La ligne 10563 contient `cp $05` qui est le début de EntityData10. EntityData08 se termine 
│ L'indentation est incorrecte. Regardons comment c'est vraiment indenté:
│ Je vois que l'indentation utilise des espaces. Je vais vérifier exactement combien:
│ 4 espaces d'indentation. Je vais utiliser l'approche du Write tool pour remplacer complètement la zo
│ Comme c'est trop complexe, je vais utiliser une approche plus simple: créer un fichier temporaire av
│ Je vais utiliser une approche différente: créer un script Python pour faire le remplacement directem
│ Excellent! Vérifions le résultat et remplaçons le fichier:
│ Parfait! Maintenant, vérifions que le binaire recompile correctement avec `make verify`:
│ Il y a une erreur! Le hash est différent. Vérifions quelle différence il y a:
│ Les données ont l'air correctes! Vérifions Entity Data08:
│ Aussi correct! Le problème doit être ailleurs. Vérifions s'il y a d'autres changements:
│ Je vois le problème! Le commentaire d'EntityData08 apparaît deux fois. Je dois corriger ça:
│ Je vois, il y a un doublon du commentaire. Corrigeons ça:
│ Excellent! Maintenant vérifions à nouveau:
│ Le hash est toujours différent. Vérifions plus en détail ce qui a changé. Il peut y avoir un problèm
│ Attendez, ce n'est pas bon! Les données à $7AB2 devraient être `02 3B 5B A2 5E` mais j'ai `3B 5B 5E 
│ Je vois, il manque des lignes! Le problème est que mon script Python a coupé à la ligne 10563, mais 
│   "summary": "Données d'entités pour le niveau 3 (type 09), 207 bytes de commandes de positionnement
│ }
│ ```
