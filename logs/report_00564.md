...
<CommentaireGit>
[BFS-5179] Map data niveau 5
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index 2d4bfef..4840817 100644
...
+    dw $54D5, Level5MapData, $5222
     ...
     dw $529B, $5311, $5405
     ; Niveau 7
@@ -2827,159 +2827,31 @@ ResetGameStateInit:
...
<ReflexionAgent>
 ...
 │ Je vais analyser le nœud à l'adresse $55E2 (données de map partagées pour les niveaux 0-2) dans la b
│ L'adresse $55E2 est référencée comme pointeur dans des structures de données. Laissez-moi chercher l
│ Je vois que $55E2 est référencé comme pointeur de map data pour les niveaux 0, 1, et 2. Laissez-moi 
│ Les données à $55E2 semblent être des données de map (valeurs de tiles). Laissez-moi lire plus du fi
│ L'adresse $55E2 est utilisée comme pointeur mais n'a pas encore de label défini. Je dois chercher au
│ Je dois chercher plus loin. Calculons mieux: la bank 1 commence à $4000. $55E2 - $4000 = $15E2 = 560
│ Je dois avancer plus loin. Cherchons avec une approche différente - recherchons les labels autour de
│ Le fichier fait 12541 lignes. Cherchons directement autour de l'adresse $55E2. Lisons autour de la l
│ - Fin: $55BB + $27 = $55E2
│ Donc $55E2 est exactement où commence la zone mal désassemblée après SharedTilesetData_024! Continuo
│ Excellent! J'ai trouvé la zone. Les lignes 3385 à 3457 sont clairement des données mal désassemblées
│ - Taille: $5605 - $55E2 = $23 = 35 octets
│ Regardons maintenant $5605 qui est la référence suivante dans la LevelJumpTable (entities data):
│ Le bloc à $5605 se termine aussi par $FF à $562F (on voit $FF à offset $562F-$5605=$2A). Maintenant,
│ Bien! Maintenant mettons à jour la référence dans LevelJumpTable pour utiliser le nouveau label:
│ Parfait! Maintenant vérifions avec `make verify`:
│ Il y a une erreur! J'ai probablement mal interprété les données. Vérifions avec xxd exactement ce qu
│ Ah! Je vois l'erreur. À l'offset $55E4, il y a `5A48` et non `485A`. Je dois respecter l'ordre littl
│ Parfait! Maintenant je peux reconstruire correctement. Les données en little-endian sont:
│ - $55E6: 5A 48 → dw $485A ✓
│ Donc ma reconstruction était correcte! Le problème doit être ailleurs. Laissez-moi comparer exacteme
│ - ...
│ Donc l'ancien code contenait plus d'octets! Regardons ce qui vient après le $FF à $5604:
│ Ah! Je vois! À $5604 il y a `FF CD`, pas juste `FF`. Le `FF` termine SharedMapData_012, mais immédia
│ Donc mes données sont correctes, mais les données qui suivent (à partir de "call $2756") doivent aus
│ J'ai supprimé plus d'octets que j'en ai ajouté. Laissez-moi vérifier ce que l'ancien code désassembl
│ Laissez-moi calculer les octets de ce code désassemblé:
│ - CD 56 BB 5A 48 60... jusqu'à 5F 5A FF
│ Donc mon code est correct. Mais pourquoi le hash change-t-il? Ah! Je pense que j'ai aussi supprimé d
│ Cette section après le premier `rst $38` (qui est le `FF` à $5604) est la zone des entities à $5605.
│ - Mon code test: `5a48` = `ZH`
│ Attendez, dans un des xxd précédents j'ai vu `5a48` mais dans un autre `485a`. Laissez-moi re-vérifi
│ Ah, game.gb n'est pas dans git (c'est un fichier généré). Donc il n'y a pas d'original à comparer. L
│ Bon, les données à $55E2 sont identiques! Mais le hash global change. Cela signifie que quelque chos
│ Le fichier n'a pas été créé car la compilation a échoué dans `make verify`. Laissez-moi regarder le 
│ Ah, game.gb n'existe plus car `make verify` a échoué. Je dois recompiler proprement:
│ Je suis déjà dans src! Donc:
│ Bon, il compile. Maintenant comparons les octets:
│ Oh attendez! Le warning dit "Overwrote a non-zero byte in the global checksum". Cela pourrait être l
│ Laissez-moi vérifier si j'ai fait une erreur dans mes données. Comparons avec un backup de l'origina
│ Bien, l'original compile. Maintenant sauvegardons le game.gb original et restaurons mes modification
│ Mais attendez, mes dw disent `$485A`! Donc MON code génère `5A 48` ce qui est correct. Mais l'origin
│ Donc il y a une erreur dans ma lecture du xxd original! Laissez-moi vérifier le fichier original:
│ Voilà! À $55E6, l'original a `48 60` = `$6048` et PAS `$485A`! J'ai fait une erreur en lisant les do
│ Parfait! Maintenant vérifions:
│ Excellent! `make verify` passe! Maintenant nettoyons les fichiers temporaires et analysons les référ
│ Maintenant, analysons les données pour identifier les références sortantes. SharedMapData_012 contie
│   "summary": "Table de pointeurs vers tiles graphiques pour le layout de map partagé entre les nivea
│ }
│ ```
