...
<CommentaireGit>
[BFS-704E] Animation frame structure
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index aca9537..8a033e6 100644
...
index 26bf6b9..1b96ed3 100644
--- a/src/bank_003.asm
+++ b/src/bank_003.asm
@@ -10878,9 +10878,15 @@ PaddingZone_003_6f30:
     ld a, b
     sbc c
...
+    sbc c
+
+; AnimationFrameData_703C / HandleAudioConditionalLogic (ancien label)
+; ----------------------------------------------------------------------
+; Description: Structure de données d'animation avec sprites et coordonnées
+; Format:
+;   - FD XX = Marqueur spécial/flags
+;   - FE XX = Marqueur de frame (XX = nombre d'éléments)
+;   - Données sprite: séquences de bytes (tile ID, coords Y/X, attribut)
+; In:  Référencé par animation handler à $4C37
+; Out: Données de frames d'animation pour le moteur de sprites
+; Modifie: Utilisé par le renderer de sprites
+; Note: $704E (+0x12 depuis début) fait partie de cette structure
+;       Zone précédemment mal désassemblée comme du code
+AnimationFrameData_703C:
+HandleAudioConditionalLogic:  ; Alias (zone de données, pas code exécutable)
+    db $FD, $67, $FE, $08, $32, $67, $67, $67  ; $703C: Début frame + flags
+    db $82, $82, $82, $82, $A6, $FD, $67, $FE  ; $7044: Sprite data
+    db $02, $31                                 ; $704C: Frame marker
+AnimationFrameData_704E:  ; Point d'entrée spécifique référencé par $4C37
+    db $67, $71, $67, $F1, $67, $FE  ; $704E: Sprite coords/tile
+    db $02, $32, $67, $71, $67, $F1, $67, $FE  ; $7054: Suite frame
+    db $02, $31, $67, $72, $FD, $67, $F1, $67  ; $705C: Sprite data + flags
+    db $FE, $02, $32, $67, $41, $82, $76, $F4  ; $7064: Frame + sprite coords
+    db $67, $6D, $6D, $6D, $6B, $F1, $67, $FE  ; $706C: Sprite data
+    db $02, $31, $67, $41, $81, $85, $67, $6E  ; $7074: Frame marker + coords
+    db $6E, $6E, $6C, $F1, $67, $FE, $02, $32  ; $707C: Sprite data + frame
+    db $67, $41, $80, $72, $F4, $67, $F1, $67  ; $7084: Coords + sprite data
+    db $FE, $02, $31, $67, $41, $81, $81, $67  ; $708C: Frame + coords
+    db $D3, $70, $72, $67, $FE, $02, $32        ; $7094: Data + frame marker
+
...
│ Pas de résultat direct. L'adresse $7372 est en bank 3, donc l'adresse physique ROM est $C000 + $7372
│ Le fichier est trop gros. Cherchons le contexte autour de l'adresse $7372:
│ Calculons l'offset ROM correct. Bank 3 commence à $C000 dans la ROM, et $7372 est l'adresse logique.
│ Utilisons l'agent Explore pour trouver cette adresse:
│ Parfait ! L'adresse $7372 se trouve dans une zone qui semble mal désassemblée. Les instructions `and
│ Excellent ! Comparons avec les données brutes pour comprendre la structure:
│ - `40 42` répété - pattern de paires
│ Regardons ce qui référence cette zone:
│ Il y a des sauts vers $7377 et $7383. Lisons le contexte avant $7370:
│ Regardons aussi ce qui vient après pour mieux comprendre:
│ - Des valeurs ASCII: `58 54 52 4e 4a` = "XTRNJ"
│ C'est clairement une **table de données d'animation**. Cherchons les adresses référencées:
│ ```
│ Les données semblent être organisées en paires de bytes. Reconstruisons cette zone:
│ Je dois d'abord lire une version complète de la zone à modifier:
│ Maintenant je peux reconstruire. Vérifions d'abord les valeurs exactes du dump:
│ Parfait ! Maintenant je reconstruis la zone complète. L'adresse $7372 est à l'offset +2 du début ($7
│ Excellent ! Maintenant vérifions avec `make verify`:
│ Ah! Il y a des références vers ces labels qui ont été supprimés. Je dois trouver toutes les référenc
│ 2. Des `jr c/z, PaddingZone_...` (lignes 11478, 11484, 11559)
│ Les références à `ProcessInputState_Bank3_Part2` pointent vers $7370, qui est maintenant `AnimationF
│ Les `call nz` ne semblent pas logiques ici non plus. Vérifions le dump:
│ Je vois: `cd 59 6b` = `call $6b59`. Mais dans l'assembleur, cela a été traduit comme `call nz, Proce
│ Je comprends maintenant ! Les `call ProcessInputState_Bank3_Part2` pointent en réalité vers l'adress
│ Maintenant vérifions si le build passe:
│ Le hash est différent ! Cela signifie que ma reconstruction ne produit pas le même binaire. Vérifion
│ ```
│ Le problème est que j'ai écrit `db $70, $a0` alors que ce devrait être juste `db $a0`. Vérifions à n
│ Annulons les changements et repartons de zéro en comparant byte par byte:
│ Le game.gb n'est pas versionné. Reconstruisons avec la version originale:
│ Bon. Maintenant regardons précisément ce qui est dans le source original et comparons avec le dump h
│ - Ligne 11521: `and e` = opcode `$a3` ✓ correspond à $F373
│ Donc l'adresse $7370 contient l'opcode `$70` qui n'est PAS montré! Regardons avant:
│ - $7370: `and b` ($a0) ← Le label pointe ici!
│ Maintenant je comprends. Je dois reconstruire EXACTEMENT le même code en respectant les opcodes. Ref
│ Donc $F370 contient bien `$70`. Mais ce `$70` correspond à l'instruction `ld [hl], b` ligne 11516, P
│ Le label `ProcessInputState_Bank3_Part2` sur la ligne 11518 pointe vers l'adresse juste APRÈS `ld [h
│ Donc l'adresse $7370 devrait contenir `$a0`, pas `$70`! Il y a un décalage quelque part. Vérifions l
│ - $7372 = `nop` ($00) ← L'ADRESSE DEMANDÉE!
│ Donc l'adresse $7372 est bien un `nop` ($00) qui est un terminateur dans les données d'animation. Ma
│ Maintenant vérifions:
│ Vérifions ce qui a été généré:
│ Parfait! Les bytes sont identiques ET les labels pointent vers $7371. Le hash est différent mais les
│ Les données semblent identiques! Cherchons où est la différence:
│ Il n'y a pas de fichier de référence. Sauvegardons le binaire original avant modification:
│ Maintenant refaisons la modification et comparons:
│ - `ld d, b` = `$50`
│ Je vais juste ajouter des commentaires au code existant sans le restructurer, car toute modification
│ Parfait ! Maintenant vérifions:
│ Oh non ! J'ai ajouté trop de commentaires et la section est devenue trop grande (1 byte de trop). Le
│ Je vois ! J'ai ajouté un NOUVEAU label `AnimationFrameData_7371:` en PLUS de `ProcessInputState_Bank
│ Toujours +1 byte. C'est bizarre. Regardons tous les labels que j'ai ajoutés:
│ J'ai ajouté 4 nouveaux labels! Chaque label ne devrait pas prendre de place dans le binaire final...
│ Donc j'ai ajouté les mêmes labels + 4 nouveaux. Les 4 nouveaux doivent être des aliases, pas de nouv
│ Je vois le problème : ligne 11607, `PaddingZone_003_73c1:` ET `AnimationFrameData_73be:` sont deux l
│ Maintenant testons:
│ Toujours +1 byte! Le problème doit venir d'ailleurs. Comptons le nombre de lignes de code ajoutées v
│ AH! J'ai 79 lignes de code ajoutées vs 78 supprimées = +1 ligne de code! Cherchons laquelle:
│ 78 lignes supprimées, mais j'ai ajouté `ld e, b` quelque part! Cherchons:
│ J'ai 2 `ld e, b` ajoutés! Dans l'original il n'y en a qu'un. Regardons le contexte:
│ ```
│ Le `ld e, b` fait partie de la séquence XTRNJ qui vient APRÈS `ld bc, $58a1`. Je l'ai dupliqué! Corr
│ - `$58` (de `ld e, b`)
│ Mais ça fait 4 bytes au lieu de 3! `ld bc, $58a1` compile déjà le `$58` ! Donc j'ai dupliqué. Il ne 
│ Parfait! Maintenant testons:
│ 🎉 **SUCCÈS !** Le hash est identique! Maintenant, préparons le JSON de sortie avec les références dé
│ - **$73d4** : Pointeur vers autre frame
│ Le fichier src/bank_003.asm:11527 contient maintenant une documentation complète de cette zone.
