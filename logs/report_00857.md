...
<CommentaireGit>
[BFS-7282] Pattern audio - Séquence de bytes audio non encore explorée
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index 5cf05ac..47304f8 100644
...
+      "address": "$FFFF",
+      "node_type": "data",
+      "description": "Terminateur audio (marqueur de fin de s\u00e9quence)",
+      "source": "$7282",
+      "bank": 3,
+      "priority": 3
     }
...
index e195c8d..877d1cc 100644
--- a/src/bank_003.asm
+++ b/src/bank_003.asm
@@ -11815,8 +11815,19 @@ ProcessInputState_Bank3_Part1:
     ld b, $01
     db $10
     ld bc, $0006
-    sub h
...
+; -------------------------
+; Description: Pattern audio pour séquence musicale #3
+; Format: Données audio mal désassemblées comme du code (118 bytes)
+; In:  Référencé par AudioMusicSequence_70B5[2]
...
+    ld [hl], d                 ; $7283: $72
     ...
     rst $38
     add d
diff --git a/src/bank_003.asm.failed b/src/bank_003.asm.failed
new file mode 100644
index 0000000..b5a8426
--- /dev/null
+++ b/src/bank_003.asm.failed
@@ -0,0 +1,14761 @@
...
+    dec bc
+
+; AudioSequencePattern_7270 / ProcessInputState_Bank3_Part1
+; ----------------------------------------------------------
+; Description: Pattern audio pour séquence musicale #3
+; Format: 18 bytes de données audio brutes
+; In:  Référencé indirectement par AudioMusicSequence_70B5
+; Utilisation: Données audio lues par le moteur sonore
+; NOTE: Le label ProcessInputState_Bank3_Part1 est un ALIAS HISTORIQUE (données mal
+;       désassemblées comme code). Les "call nz, ProcessInputState_Bank3_Part1" dans
+;       le code sont en réalité des DONNÉES (db $C4, $70, $72) mal désassemblées.
+ProcessInputState_Bank3_Part1:  ; ALIAS - Ce ne sont PAS des instructions!
+AudioSequencePattern_7270:
+    db $6A, $69, $6A, $E2, $69, $6A, $FE, $04, $32, $69, $6A, $69, $B1, $6F, $E2, $6A, $69, $FE
+
+; AudioSequencePattern_7282
+; -------------------------
+; Description: Pattern audio #2 pour séquence musicale #3
+; Format: 6 bytes de données audio brutes
+; In:  Référencé par AudioMusicSequence_70B5[2]
+; Utilisation: Données audio lues par le moteur sonore
+AudioSequencePattern_7282:
+    db $04, $31, $6A, $69, $6A, $E2
+
+; AudioSequencePattern_7288
+; -------------------------
+; Description: Pattern audio #3 pour séquence musicale #3
+; Format: 6 bytes de données audio brutes
+; In:  Référencé par AudioMusicSequence_70B5[3]
+; Utilisation: Données audio lues par le moteur sonore
+AudioSequencePattern_7288:
+    db $69, $6A, $FE, $04, $32, $69
+
+; AudioSequencePattern_728E
+; -------------------------
+; Description: Pattern audio #4 pour séquence musicale #3
+; Format: Données audio brutes (taille variable)
+; In:  Référencé par AudioMusicSequence_70B5[5]
+; Utilisation: Données audio lues par le moteur sonore
+AudioSequencePattern_728E:
+    db $6A, $69, $E2, $6A, $69, $FE, $05, $31, $6A, $69, $6A, $69
+    db $E2, $69, $6A, $FE, $05, $32, $69, $6A, $69, $6A, $E2, $6A
+    db $69, $FE, $05, $31, $6A, $69, $6A, $69, $E2, $69, $6A, $FE
+    db $05, $32, $7F, $6A, $7F, $6A, $E2, $6A, $7F, $FE, $05, $31
+    db $7F, $69, $7F, $69, $D1, $7F, $F1, $5D, $FE, $05, $32, $69
+    db $6A, $69, $6A, $D1, $7F, $F1, $5D, $FE, $05, $31, $6A, $69
+    db $6A, $69, $D1, $6F, $F1, $5D, $FE, $05, $32, $69, $6A, $69
+    db $6A, $D1, $7F, $F1, $5D, $FE, $05, $31, $6A, $69, $6A, $69
+    db $F1, $5D, $FE, $05
+
...
index 4b7dca3..abb35db 100644
Binary files a/src/game.gb.ref and b/src/game.gb.ref differ
...
<ReflexionAgent>
 ...
 ^[[C^[[C^[[C^[[C│ - 📝 Documentation et nommage du pattern découvert
│ Commençons par localiser ce code dans les sources.
│ Parfait ! Le label existe déjà. Lisons maintenant le contexte complet de cette séquence.
│ Bien ! Le pattern $728E existe et est déjà correctement documenté. Je vois qu'il référence $72D1. Vé
│ $72D1 n'a pas encore de label. Trouvons cette adresse dans le fichier.
│ Je vois que $72D1 pointe vers des données audio brutes. Cherchons plus précisément cette zone dans l
│ Maintenant je comprends mieux. Le pattern $728E est déjà bien analysé. Il pointe vers $72D1 qui semb
│ Excellent ! Je vois maintenant que les données à partir de $72D1 ne sont PAS du code mais bien des d
│ Bien ! Je vois que cette zone est déjà identifiée comme "mal désassemblée". Les lignes 11895-11901 c
│ Parfait ! Maintenant je vais lire une plus grande section pour bien comprendre toute la structure et
│ Maintenant je vais ajouter le label à la ligne 11894 (qui correspond à $72D1 d'après mes calculs). V
│ Excellent ! La ligne 11894 correspond bien à $72D1. Maintenant je vais ajouter un label et un commen
│ Parfait ! Maintenant mettons à jour la référence dans le pattern $728E pour utiliser le nouveau labe
│ Bien ! Maintenant je dois aussi mettre à jour l'auto-référence $728E pour utiliser le label. Mais d'
│ Parfait, aucune autre référence. Maintenant vérifions la compilation avec `make verify`.
│ 2. **AudioSequencePattern_728E** (lui-même, $728E) - boucle
│ Regardons maintenant si les données à $72D1 contiennent des pointeurs vers d'autres zones.
│ Je vois que les données se terminent avec `rst $38` (FFFF) qui semble être un marqueur de fin. Regar
│ - `F7 72` = $72F7
│ Vérifions si ce sont vraiment des pointeurs en little-endian.
│ - $731B, $72F7
│ Cherchons ces adresses dans le code source.
│ Seul $735A a un label (PaddingZone). Les autres adresses n'ont pas de labels. Vérifions si elles son
│ 1. **$72D1** (AudioDataRaw_003_72d1) - données audio brutes qui contiennent elles-mêmes des pointeur
│ Préparons maintenant le JSON final.
│   "summary": "Table de 3 words définissant un pattern audio : pointeur vers données brutes ($72D1), 
│ }
│ ```
