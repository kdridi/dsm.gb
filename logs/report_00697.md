...
<CommentaireGit>
[BFS-6564] Animation frame structure
</CommentaireGit>

Le diff git
<DiffGit>
diff --git a/scripts/bfs_state.json b/scripts/bfs_state.json
index 04aada4..26e69b9 100644
...
diff --git a/src/bank_003.asm.bak b/src/bank_003.asm.bak
new file mode 100644
index 0000000..f6efb02
--- /dev/null
+++ b/src/bank_003.asm.bak
@@ -0,0 +1,14281 @@
...
+; Tilemap_510B
+; ----------------
+; Description: Table de données pour tilemap du contexte 11
+; Format: Séquence de words (valeurs 16-bit)
+Tilemap_510B:
+    dw $0200, $0200, $3800, $C400
+    dw $0100, $0200, $8300, $0700
+    dw $0F00, $3200, $4000
+
+_6564:  ; Point d'entrée spécifique - début de sous-table?
+    dw $8000  ; Masque bit 15
+    dw $0100  ; Masque bit 0
+
+; AudioTable_Block1
+; ----------------
+; Description: Table de masques binaires (puissances de 2)
+; Format: Séquence de words pour manipulation de bits
+AudioTable_Block1:
+    dw $0100, $0200, $0400, $0800, $1000, $2000
+    dw $FBFD, $FBFD, $FBFD, $FBFD, $FBFD, $FBFD, $FBFD, $FBFD
+    dw $0300
+
+; AudioTable_Block2
+; ----------------
+; Description: Table de données audio - valeurs de configuration
+AudioTable_Block2:
+    dw $9900, $9900, $8100, $FF00, $9900, $C100, $4200, $7E00
+    dw $0000, $0000
+
+; AudioTable_Block3
+; ----------------
...
│ Pas de label défini à $66A1. Cherchons la section où se trouve cette adresse.
│ Bon, $66A1 est référencé dans Level3TilesetData mais n'a pas encore son propre label/section. Cherch
│ Cherchons différemment - regardons les données brutes pour comprendre le format.
│ Le pattern montre une structure répétitive avec `fe` (254) comme séparateur. Cherchons dans le fichi
│ Utilisons une meilleure approche - calculons l'offset dans le fichier.
│ Cherchons les labels autour de cette zone - regardons entre $65D3 et $67BF:
│ Regardons directement dans le fichier ASM les lignes autour de la zone mal désassemblée:
│ Cette zone est mal désassemblée. L'adresse $66A1 se situe dans une zone de données compressées. Cont
│ C'est encore du code mal désassemblé (pattern `cp $02`, `ld [hl], e` répétés = data). Cherchons les 
│ Cherchons autrement - regardons toutes les sections:
│ Ah! Il n'y a qu'une seule SECTION dans bank_001.asm. Tout le code est dans une seule section qui com
│ Calculons la ligne exacte en cherchant des marqueurs connus:
│ Parfait! Les données commencent par `04 31 7f 7f 7f 52 34 35 e2 65 66 fe`. Cherchons ce pattern dans
│ Cherchons différemment - utilisons les bytes successifs:
│ Bingo! `ld sp, $7f7f` correspond à `31 7f 7f`. Regardons autour de la ligne 6690:
│ Mais attendez - le xxd montre `04 31 7f 7f ...`, donc c'est plutôt `inc b` (`04`) suivi de `ld sp, $
│ Regardons un peu avant:
│ Le `cp $04` ligne 6689 = `FE 04`, donc ce qui précède doit être $66A1. Vérifions:
│ En fait, $66A1 commence par `04 31 7f 7f...` donc c'est le début d'un nouveau bloc de tile data. La 
│ Cherchons où débute exactement cette zone. Remontons pour trouver un label ou commentaire:
│ Continuons à remonter pour trouver un label:
│ Parfait! Regardons ce label et sa zone:
│ Excellent! Je vois que TileGraphic_65D3 commence à la ligne 6532, avec les premiers bytes correcteme
│ Regardons la taille totale de cette zone. D'après xxd, les données semblent être organisées en blocs
│ Parfait! La structure est claire : des blocs de données séparés par `FE`. Analysons plus en détail p
│ Pas de labels dans game.sym pour cette zone. Cherchons dans le fichier ASM lui-même les prochains la
│ Donc entre TileGraphic_65D3 (ligne 6532) et ValidatePlayerState_69fd (ligne 7366), tout est mal désa
│ C'est une grosse zone! Ma mission est de documenter spécifiquement $66A1. Regardons cette adresse da
│ Le pattern à $66A1 est: `04 31 7f 7f 7f 52 34 35 e2 65 66 fe` (12 bytes), puis le prochain bloc comm
│ Analysons le format complet de cette zone:
│ - $66A1: début du bloc 20
│ Vérifions:
│ - $66A1-$66AC: `04 31 7f 7f 7f 52 34 35 e2 65 66 FE` = 12 bytes (bloc 20)
│ Donc $66A1 est le DÉBUT du 21ème bloc (après 20 FE), pas du bloc lui-même! Recalculons:
│ - Ce bloc contient des références internes potentielles (`E2 65 66` = adresse $6566)
│ Identifions toutes les références dans ce bloc:
│ Analysons le pattern complet pour comprendre le format. Le byte `E2` semble être une commande/marque
│ - Le format semble être: `[size] [data...] [E2 addr_lo addr_hi | E8]`
│ Regardons maintenant le bloc à $66A1:
│ Attendez, j'ai inversé! En little-endian, `65 66` = $6665, pas $6566. Corrigeons:
│ Bon, donc le bloc à $66A1 référence l'adresse $6665 (aussi dans la zone TileGraphic_65D3). Maintenan
│ Trouvons la ligne exacte dans bank_001.asm:
│ - `04` = début TileData_66A1
│ Reconstruisons avec les bytes corrects:
│ ```
│ Reconstruisons:
│ Excellent! Maintenant testons que le binaire est identique:
│ Erreur! Le hash ne correspond pas. Je dois vérifier mes bytes. Regardons ce que j'ai généré vs ce qu
│ Attendez, les bytes générés ne correspondent pas! À $66A1, je devrais avoir `04 31 7F 7F...` mais j'
│ Le problème est que j'ai remplacé trop de lignes. Vérifions exactement quels bytes correspondaient a
│ Ah! Le `cp $00` était en réalité `FE 00`, donc le bloc se termine à $66AC avec `FE`, et le byte `00`
│ La ligne 6707 `ld [hl-], a` = $32. Vérifions:
│ Attendez! Les bytes après $66AC sont `00 32 7F...` dans l'original, mais mon xxd montre `32 7F...`. 
│ Ah! L'original a `FE 32` à $66AC-$66AD, pas `FE 00 32`. Donc le `cp $00` n'existe pas! Récapitulons:
│ - $66AD-...: `32 7F...` (début bloc suivant)
│ Donc mon code était correct mais j'ai mal interprété `cp $00`. En fait, le `cp $00` dans le désassem
│ Regardons le désassemblage d'origine complet:
│ - $66AD: `32` (byte suivant, PAS `00`)
│ Donc le `cp $00` n'est PAS à cet endroit! Vérifions la ligne suivante dans l'original:
│ Mais xxd montre `FE 32` à $66AC-$66AD, pas `FE 00 32`!
│ Il y a une incohérence. Rebuildonsen et vérifions:
│ Le build a échoué. Vérifions les erreurs:
│ Les fichiers n'ont pas été générés. Regardons l'erreur complète:
│ Il y a eu un changement de répertoire. Revenons au code et restaurons les modifications:
│ Ah je suis déjà dans src/. Restaurons:
│ Je dois corriger mon approche. Le problème est que `cp $00` = `FE 00`, mais dans les données réelles
│ Maintenant analysons correctement. Les bytes `cp $00` à la ligne 6698 correspondent à `FE 00`, mais 
│ `04 31 7F 7F 7F 52 34 35 E2 65 66 FE 00`
│ Reconstruisons correctement:
│ Parfait! Maintenant vérifions que le hash est correct:
│ Excellent! Le hash est identique. Maintenant produisons le JSON final:
│   "summary": "Bloc de 13 bytes de tile data avec référence interne à $6665, utilisé 6 fois par Level
│ }
│ ```
